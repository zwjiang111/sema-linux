...
static struct mutex boardinfo_lock;
...
static int get_voltage_value_ec(unsigned char ch, uint16_t *pValue)
{
    int ret;
    uint16_t buff_hm=0;
...
static int get_voltage_value_bmc(unsigned char ch, int *pValue)
{
...
static int get_voltage_description_ec(const char *Buffer, uint8_t *ch)
{
    uint8_t i,len;		
    len = strlen(Buffer);
...
static int get_voltage_description_bmc(unsigned char Ch, char *Buffer)
{
...
static int get_voltage_ec(const char *cmp, uint16_t* pValue)
{
    unsigned char ch=0;

    if(get_voltage_description_ec(cmp, &ch) == 0)
    {
	get_voltage_value_ec(ch, pValue);
	debug_printk("ch %d %d\n", ch, *pValue);
	return 0;
    }
...
static int get_voltage_bmc(char ch,int stsize, const char **cmp, uint32_t* pValue)
{
        unsigned char k;
        int res;
        ...
        {
                get_voltage_description_bmc(ch, buffer);
        }
...
static ssize_t sysfs_store_error_log(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	int ret;
        ...
        ret = adl_bmc_i2c_write_device(NULL, ADL_BMC_CMD_GET_BOARDERRLOG, 1, (void *)&pos);
        if (ret < 0){
		mutex_unlock(&boardinfo_lock);
                return ret;
	}
...
static ssize_t sysfs_store_exc_des(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
    ...
    {
	int ret;
        ret = adl_bmc_i2c_write_device(NULL, ADL_BMC_CMD_EXC_CODE_TABLE, 1, (void *)&exc_code);
        if (ret < 0){
		mutex_unlock(&boardinfo_lock);
                return ret;
	}
...
static ssize_t board_type_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    char buff[32];
    memset(buff, 0, sizeof(buff));
...
static ssize_t mac_address_ext_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    int ret;
    char buff[MAX_PATH];
    memset(buff, 0, sizeof(buff));
...
static ssize_t voltage_gfx_vcore_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    ...
    if(adl_dev->con_type == EC)
    {
    	...
    	{
        	ret = get_voltage_ec(cmpstr[i], &voltage);
    ...
    {
	int Value = 0;
	for(i = 0;i < ADL_BMC_MAX_HW_MTR_INPUT; ++i)
        {
                ret = get_voltage_bmc(i,sizeof(cmpstr)/sizeof(cmpstr[0]), cmpstr, &Value);
                if (ret == 0)
                        break;
        }
	if(ret){
...
struct kobj_attribute attr40 = __ATTR_RO(mac_address_ext);
struct kobj_attribute attr41 = __ATTR_RO(board_type);
...
static int boardinfo_probe(struct platform_device *pdev)
{
    ...
    ret = sysfs_create_file(kobj_ref, &attr41.attr); 
    if (ret < 0)
        goto ret_err;
    
    mutex_init(&boardinfo_lock);
...
#endif
{
    ...
    sysfs_remove_file(kernel_kobj, &attr41.attr); 
    kobject_put(kobj_ref);
