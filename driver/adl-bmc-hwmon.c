		...
		char *buf)
{
	...
	{
		if(temp_index == 0)
			size = adl_bmc_ec_read_device(cmd_ec+TMP_LVL_1_INDEX, (u8*)trig_temp, 1, EC_REGION_1);
		else if(temp_index == 1)
			size = adl_bmc_ec_read_device(cmd_ec+TMP_LVL_2_INDEX, (u8*)trig_temp, 1, EC_REGION_1);
		else if(temp_index == 2)
			size = adl_bmc_ec_read_device(cmd_ec+TMP_LVL_3_INDEX, (u8*)trig_temp, 1, EC_REGION_1);
		else if(temp_index == 3)
			size = adl_bmc_ec_read_device(cmd_ec+TMP_LVL_4_INDEX, (u8*)trig_temp, 1, EC_REGION_1);
		...
		const char *buf, size_t count)
{
	...
	if(hwmon_data->adl_dev->con_type == EC)
	{
		...
		if(temp_index == 0)
			size = adl_bmc_ec_write_device(cmd_ec+TMP_LVL_1_INDEX, (u8*)trig_temp, 1, EC_REGION_1);
		else if(temp_index == 1)
			size = adl_bmc_ec_write_device(cmd_ec+TMP_LVL_2_INDEX, (u8*)trig_temp, 1, EC_REGION_1);
		else if(temp_index == 2)
			size = adl_bmc_ec_write_device(cmd_ec+TMP_LVL_3_INDEX, (u8*)trig_temp, 1, EC_REGION_1);
		else if(temp_index == 3)
			size = adl_bmc_ec_write_device(cmd_ec+TMP_LVL_4_INDEX, (u8*)trig_temp, 1, EC_REGION_1);
	}
	else
	{
		int i;
		...
		char *buf)
{
	...
	{
		if(pwm_index == 0)
			size = adl_bmc_ec_read_device(cmd_ec+PWM_LVL_1_INDEX, (u8*)trig_pwm, 1, EC_REGION_1);
		else if(pwm_index == 1)
			size = adl_bmc_ec_read_device(cmd_ec+PWM_LVL_2_INDEX, (u8*)trig_pwm, 1, EC_REGION_1);
		else if(pwm_index == 2)
			size = adl_bmc_ec_read_device(cmd_ec+PWM_LVL_3_INDEX, (u8*)trig_pwm, 1, EC_REGION_1);
		else if(pwm_index == 3)
			size = adl_bmc_ec_read_device(cmd_ec+PWM_LVL_4_INDEX, (u8*)trig_pwm, 1, EC_REGION_1);
		...
		const char *buf, size_t count)
{
	...
	if(hwmon_data->adl_dev->con_type == EC)
	{
		...
		if(pwm_index == 0)
			size = adl_bmc_ec_write_device(cmd_ec+PWM_LVL_1_INDEX, (u8*)trig_pwm, 1, EC_REGION_1);
		else if(pwm_index == 1)
			size = adl_bmc_ec_write_device(cmd_ec+PWM_LVL_2_INDEX, (u8*)trig_pwm, 1, EC_REGION_1);
		else if(pwm_index == 2)
			size = adl_bmc_ec_write_device(cmd_ec+PWM_LVL_3_INDEX, (u8*)trig_pwm, 1, EC_REGION_1);
		else if(pwm_index == 3)
			size = adl_bmc_ec_write_device(cmd_ec+PWM_LVL_4_INDEX, (u8*)trig_pwm, 1, EC_REGION_1);
	}
	else
	{
		u8 trig_pwm[32];
...
static ssize_t show_temp_input(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	switch(ix) 
	{
		case 0:
			if(hwmon_data->adl_dev->con_type == EC)
			{
				ret = adl_bmc_ec_read_device(ADL_BMC_OFS_RD_CPU_TEMP, &temp_buff, 1, EC_REGION_1);	
				temper = encode_celcius ((char)temp_buff);
				debug_printk("%s ix %d buff %d temper %d\n", __func__, ix, temp_buff, temper);
			}
			else
			{
		...
		case 1:
			if(hwmon_data->adl_dev->con_type == EC)
			{
				ret = adl_bmc_ec_read_device(ADL_BMC_OFS_RD_BOARD_TEMP, &temp_buff, 1, EC_REGION_1);
				temper = encode_celcius ((char)temp_buff);
				debug_printk("%s ix %d buff %d temper %d\n", __func__, ix, temp_buff, temper);
			}
			else
		...
		case 2:
			if(hwmon_data->adl_dev->con_type == BMC)
			{
                        ...
                        break;
		case 3:
			ret = adl_bmc_ec_read_device(ADL_BMC_OFS_RD_SYSTEM_TEMP, &temp_buff, 1, EC_REGION_1);
			if(ret < 0)
				goto Exit;
...
static ssize_t show_temp_min(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	switch(ix) 
	{
		case 0:
			if(hwmon_data->adl_dev->con_type == EC)
			{
				ret = adl_bmc_ec_read_device(ADL_BMC_OFS_RD_MINCPU_TEMP, &temp_buff, 1, EC_REGION_1);
				temper = encode_celcius((char)temp_buff);
				debug_printk("%s ix %d buff %d temper %d\n", __func__, ix, temp_buff, temper);
			}
			else
		...
		case 1:
			if(hwmon_data->adl_dev->con_type == EC)
			{
				ret = adl_bmc_ec_read_device(ADL_BMC_OFS_RD_MINBRD_TEMP, &temp_buff, 1, EC_REGION_1);
				temper = encode_celcius((char)temp_buff);
				debug_printk("%s ix %d buff %d temper %d\n", __func__, ix, temp_buff, temper);
			}
			else
		...
		case 2:
			if(hwmon_data->adl_dev->con_type == BMC)
			{
                        ...
                        break;
		case 3:
			ret = adl_bmc_ec_read_device(ADL_BMC_OFS_RD_MINSYS_TEMP, &temp_buff, 1, EC_REGION_1);
...
static ssize_t show_temp_max(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	switch(ix) 
	{
		case 0:
			if(hwmon_data->adl_dev->con_type == EC)
			{
				ret = adl_bmc_ec_read_device(ADL_BMC_OFS_RD_MAXCPU_TEMP, &temp_buff, 1, EC_REGION_1);
				temper = encode_celcius((char)temp_buff);
				debug_printk("%s ix %d buff %d temper %d\n", __func__, ix, temp_buff, temper);
			}
			else
		...
		case 1:
			if(hwmon_data->adl_dev->con_type == EC)
			{
				ret = adl_bmc_ec_read_device(ADL_BMC_OFS_RD_MAXBRD_TEMP, &temp_buff, 1, EC_REGION_1);
				temper = encode_celcius((char)temp_buff);
				debug_printk("%s ix %d buff %d temper %d\n", __func__, ix, temp_buff, temper);
			}
			else
		...
		case 2:
			if(hwmon_data->adl_dev->con_type == BMC)
			{
                        ...
                        break;
		case 3:
			ret = adl_bmc_ec_read_device(ADL_BMC_OFS_RD_MAXSYS_TEMP, &temp_buff, 1, EC_REGION_1);
...
static ssize_t show_temp_startup(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	switch(ix) 
	{
		case 0:
			if(hwmon_data->adl_dev->con_type == EC)
			{
				ret = adl_bmc_ec_read_device(ADL_BMC_OFS_RD_CPU_STARTUP_TEMP, &temp_buff, 1, EC_REGION_1);
				temper = encode_celcius((char)temp_buff);
				debug_printk("%s ix %d buff %d temper %d\n", __func__, ix, temp_buff, temper);
			}
			else
		...
		case 1:
			if(hwmon_data->adl_dev->con_type == EC)
			{
				ret = adl_bmc_ec_read_device(ADL_BMC_OFS_RD_BRD_STARTUP_TEMP, &temp_buff, 1, EC_REGION_1);
				temper = encode_celcius((char)temp_buff);
				debug_printk("%s ix %d buff %d temper %d\n", __func__, ix, temp_buff, temper);
			}
			else
		...
		case 2:
			if(hwmon_data->adl_dev->con_type == BMC)
			{
                        ...
                        break;
		case 3:
			ret = adl_bmc_ec_read_device(ADL_BMC_OFS_RD_SYS_STARTUP_TEMP, &temp_buff, 1, EC_REGION_1);
...
static int adl_bmc_hwmon_probe(struct platform_device *pdev)
{
	...
	if (hwmon_data->adl_dev->Bmc_Capabilities[0] & ADL_BMC_CAP_TEMP) 
	{
		...
		if(hwmon_data->adl_dev->con_type == EC || DtsTemp == 0)
		{
			for (i = 0; i < ARRAY_SIZE(adl_bmc_sysfs_cpu_temp); i++)
			{
	...
	/*check board temperature capability and create sysfs entry for board temperature*/
	if (hwmon_data->adl_dev->Bmc_Capabilities[0] & ADL_BMC_CAP_TEMP) 
	{
		for (i = 0; i < ARRAY_SIZE(adl_bmc_sysfs_board_temp); i++)
		{
