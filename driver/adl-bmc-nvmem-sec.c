...
static long ioctl(struct file *file, unsigned int cmd, unsigned long data)
{
	...
	// Validate user pointer and copy input once for all cases that use it
	if (copy_from_user(&buffer, (void __user *)data, sizeof(struct secure)) != 0)
		return -EFAULT;

	switch (cmd)
	{
		case EAPI_STOR_LOCK:
			RetVal = adl_bmc_nvmem_lock(&buffer);
			if (RetVal < 0)
				return -EINVAL;

			if (copy_to_user((void __user *)data, &buffer, sizeof(struct secure)) != 0)
				return -EFAULT;

			return 0;

		case EAPI_STOR_UNLOCK:
			RetVal = adl_bmc_nvmem_unlock(&buffer);
			if (RetVal < 0)
				return -EINVAL;
...
static int adl_bmc_nvmem_probe(struct platform_device *pdev)
{
    ...
    adlink->class = class_create("adl-bmc-nvmem-eapi");
#elif LINUX_VERSION_CODE >= KERNEL_VERSION(6,4,0)
    adlink->class = class_create("adl-bmc-nvmem-eapi");
#else
    adlink->class = class_create(THIS_MODULE, "adl-bmc-nvmem-eapi");
#endif
    ...
    if(device_create(adlink->class, NULL, adlink->ldev, NULL, "bmc-nvmem-eapi") == NULL)
    {
