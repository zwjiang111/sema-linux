#include <linux/module.h>
#include <linux/init.h>
#include <linux/platform_device.h>
#include <linux/backlight.h>
#include <linux/mfd/core.h>
#include <linux/fb.h>
#include <linux/slab.h>
#include <linux/acpi.h>
#include <linux/namei.h>
#include <linux/device.h>
#include <linux/cdev.h>
#include <linux/delay.h>
#include <linux/seq_file.h>
#include <linux/version.h>
#include <linux/pci.h>
#include <linux/i2c.h>

#include "adl-ec.h"
#include "adl-bmc.h"

#define EC_TIMEOUT_LOOPS	500

#if __has_include("/etc/redhat-release")
#define CONFIG_REDHAT
#endif

#define BMC_ADDRESS	0x50
unsigned short SMBBase = 0x00;

#define SMBHSTS			SMBBase+0x00	// Host Status
#define SMBHSTS_HBSY		0x01		// Host busy
#define SMBHSTS_INUSE_STS	0x40		// INUSE flag
#define SMBHCTL			SMBBase+0x02	// Host Control
#define SMBHCMD			SMBBase+0x03	// Host Command
#define SMBTSA			SMBBase+0x04	// Transmit Slave Address
#define SMBHD0			SMBBase+0x05	// Host Data 0
#define SMBHD1			SMBBase+0x06	// Host Data 1
#define SMBHBD			SMBBase+0x07	// Host Block Data (always read/write 32 times!)
#define SMBAUXCTL		SMBBase+0x0d	// Auxiliary control register

#define BMC_IsReadAdr(a)	(a&1)		///< Tests if address is read address
#define BMC_IsWriteAdr(a)	(!(a&1))	///< Tests if address is write address

#define	ERRORRETRY_TIMES	5

#define	INT_SMI_FAILED( VALUE )		( VALUE & 0x10 )
#define	BUS_ERROR( VALUE )		( VALUE & 0x8 )
#define	TIMEOUT_ERROR( VALUE )		( VALUE & 0x4 )

#define	SET_INT_SMI_FAILED_BIT( VALUE )		( VALUE | 0x10 )
#define	SET_BUS_ERROR_BIT( VALUE )		( VALUE | 0x8 )
#define	SET_TIMEOUT_ERROR_BIT( VALUE )		( VALUE | 0x4 )

#define	SMBUS_ERROR( VALUE )		( INT_SMI_FAILED( VALUE ) || BUS_ERROR( VALUE ) || TIMEOUT_ERROR( VALUE ) )

struct adl_bmc_dev *adl_bmc_dev;
unsigned short ec_region;

static const struct mfd_cell adl_bmc_devs[] = {
	{.name = "adl-bmc-wdt"},
	{.name = "adl-bmc-boardinfo"},
	{.name = "adl-bmc-nvmem"},
	{.name = "adl-bmc-nvmem-sec"},
	{.name = "adl-bmc-bklight"},
	{.name = "adl-bmc-vm"},
	{.name = "adl-bmc-hwmon"},
	{.name = "adl-bmc-i2c"},
	{.name = "adl-bmc-gpio"},
};

/* EC commands */
enum ec_command {
	ACPI_EC_COMMAND_READ = 0x80,
	ACPI_EC_COMMAND_WRITE = 0x81,
	ACPI_EC_BURST_ENABLE = 0x82,
	ACPI_EC_BURST_DISABLE = 0x83,
	ACPI_EC_COMMAND_QUERY = 0x84,
	ADLINK_EC_COMMAND_READ = 0xC0,
};

/* EC status register */
#define ACPI_EC_FLAG_OBF	0x01	/* Output buffer full */
#define ACPI_EC_FLAG_IBF	0x02	/* Input buffer full */
#define ACPI_EC_FLAG_CMD	0x08	/* Input buffer contains a command */
#define ACPI_EC_FLAG_BURST	0x10	/* burst mode */
#define ACPI_EC_FLAG_SCI	0x20	/* EC-SCI occurred */

struct board_info default_device = {
	"Common",
	{"RTC", "5VSB", "VIN", "3.3V", "VMEM", "3.3VSB", "VCORE","Current Input Current"},
	{ "NOERROR", NULL, "NO_SUSCLK", "NO_SLP_S5", "NO_SLP_S4", "NO_SLP_S3",\
		"BIOS_FAIL", "RESET_FAIL", "RESETIN_FAIL", "NO_CB_PWROK", "CRITICAL_TEMP",\
			"POWER_FAIL", "VOLTAGE_FAIL", "RSMRST_FAIL", "NO_VDDQ_PG", "NO_V1P05A_PG", \
			"NO_VCORE_PG", "NO_SYS_GD", "NO_V5SBY", "NO_V3P3A", "NO_V5_DUAL", "NO_PWRSRC_GD",\
			"NO_P_5V_3V3_S0_PG", "NO_SAME_CHANNEL", "NO_PCH_PG", "INVALID", "INVALID", "INVALID",\
			"INVALID", "INVALID", "INVALID", "INVALID", "INVALID", "INVALID"
	},
};

struct board_info supported_device[] = {
	{
		"EL",
		{"RTC", "5VSB", "VIN", "3.3V", "VMEM", "3.3VSB", "VCORE","Current Input Current"},
		{ "NOERROR", NULL, "NO_SUSCLK", "NO_SLP_S5", "NO_SLP_S4", "NO_SLP_S3",\
			"BIOS_FAIL", "RESET_FAIL", "RESETIN_FAIL", "NO_CB_PWROK", "CRITICAL_TEMP",\
				"POWER_FAIL", "VOLTAGE_FAIL", "RSMRST_FAIL", "NO_VDDQ_PG", "NO_V1P05A_PG", \
				"NO_VCORE_PG", "NO_SYS_GD", "NO_V5SBY", "NO_V3P3A", "NO_V5_DUAL", "NO_PWRSRC_GD",\
				"NO_P_5V_3V3_S0_PG", "NO_SAME_CHANNEL", "NO_PCH_PG", "INVALID", "INVALID", "INVALID",\
				"INVALID", "INVALID", "INVALID", "INVALID", "INVALID", "INVALID"
		},
	},
	{
		"TL",
		{"RTC", "5VSB", "VIN", "3.3V", "VMEM", "3.3VSB", "VCORE","Current Input Current"},
		{ "NOERROR", NULL, "NO_SUSCLK", "NO_SLP_S5", "NO_SLP_S4", "NO_SLP_S3",\
			"BIOS_FAIL", "RESET_FAIL", "RESETIN_FAIL", "NO_CB_PWROK", "CRITICAL_TEMP",\
				"POWER_FAIL", "VOLTAGE_FAIL", "RSMRST_FAIL", "NO_VDDQ_PG", "NO_V1P05A_PG", \
				"NO_VCORE_PG", "NO_SYS_GD", "NO_V5SBY", "NO_V3P3A", "NO_V5_DUAL", "NO_PWRSRC_GD",\
				"NO_P_5V_3V3_S0_PG", "NO_SAME_CHANNEL", "NO_PCH_PG", "INVALID", "INVALID", "INVALID",\
				"INVALID", "INVALID", "INVALID", "INVALID", "INVALID", "INVALID"
		},
	},
	{
		"AR",
		{"RTC", "5VSB", "VIN", "3.3V", "VMEM", "3.3VSB", "VCORE","Current Input Current"},
		{ "NOERROR", NULL, "NO_SUSCLK", "NO_SLP_S5", "NO_SLP_S4", "NO_SLP_S3",\
			"BIOS_FAIL", "RESET_FAIL", "RESETIN_FAIL", "NO_CB_PWROK", "CRITICAL_TEMP",\
				"POWER_FAIL", "VOLTAGE_FAIL", "NO_RSMRST_PG", "NO_VDDQ_PG", "NO_V1P05A_PG", \
				"NO_VCORE_PG", "NO_SYS_GD", "NO_V5SBY", "NO_V3P3A", "NO_V5_DUAL", "NO_PWRSRC_GD",\
				"NO_P_5V_3V3_S0_PG", "NO_SAME_CHANNEL", "NO_PCH_PG", "INVALID", "INVALID", "INVALID",\
				"INVALID", "INVALID", "INVALID", "INVALID", "INVALID", "INVALID"
		},
	}
};

void delay(unsigned long int ticks)
{
	udelay(ticks);
}
EXPORT_SYMBOL(delay);

static int wait_for_ec(uint8_t port, uint8_t mask, uint8_t cond)
{
	uint32_t i = 0;

	while(1)
	{
		/* Maximum of 5msec timeout */
		if(i > EC_TIMEOUT_LOOPS)
		{
			if(ec_region == EC_REGION_1 || ec_region == EC_REGION_2)
			{
				pr_err("Error: EC_TIMEOUT!!!\n");
			}
			return -ETIMEDOUT;
		}	
		if((inb(port) & mask) == cond)
			return 0;

		delay(10);
		i++;
	}

	return 0;
}

static int ReadEc(unsigned short int RegionIndex, unsigned short int offset, unsigned char *data)
{
	int ret;
	uint8_t Cmd;

	uint8_t EC_CTRL_PORT = (RegionIndex & 0xFF);
	uint8_t EC_DATA_PORT = ((RegionIndex >> 8) & 0xFF);
	Cmd = EC_SC_RD_CMD;

	if(ec_region != EC_REGION_1 && ec_region != EC_REGION_2)
	{
		Cmd = EC_SC_RD_CMD + 0x10;
	}

	if(RegionIndex == EC_REGION_1)
	{
		if(ec_region == EC_REGION_2)
		{
			EC_CTRL_PORT = (EC_REGION_2 & 0xFF);
			EC_DATA_PORT = ((EC_REGION_2 >> 8) & 0xFF);
			Cmd = EC_SC_RD_CMD + 0x10;
		}
	}

	if (!data){
		return -EINVAL;
	}

	/* 1. Write EC_SC_RD_CMD to control port*/
	outb(Cmd, EC_CTRL_PORT);

	/* 2. Wait for EC_SC_IBF to clear */
	if((ret = wait_for_ec(EC_CTRL_PORT, EC_SC_IBF, 0)) < 0)
		return ret;

	/* 3. Write offset address to data port */
	outb((unsigned char)offset, EC_DATA_PORT);

	/* 4. Wait for EC_SC_OBF to set */
	if((ret = wait_for_ec(EC_CTRL_PORT, EC_SC_IBFOROBF, EC_SC_OBF)) < 0)
		return ret;

	/* 5. Read data from data port */
	*data = inb(EC_DATA_PORT);

	return 0;
}

static int WriteEc(unsigned short int RegionIndex, unsigned short int offset, unsigned char data)
{
	int ret;
	uint8_t Cmd;

	uint8_t EC_CTRL_PORT = (RegionIndex & 0xFF);
	uint8_t EC_DATA_PORT = ((RegionIndex >> 8) & 0xFF);
	Cmd = EC_SC_WR_CMD;

	if(RegionIndex == EC_REGION_1)
	{
		if(ec_region == EC_REGION_2)
		{
			EC_CTRL_PORT = (EC_REGION_2 & 0xFF);
			EC_DATA_PORT = ((EC_REGION_2 >> 8) & 0xFF);
			Cmd = EC_SC_WR_CMD + 0x10;
		}
	}

	/* 1. Write EC_SC_WR_CMD to control port*/
	outb(Cmd, EC_CTRL_PORT);

	/* 2. Wait for EC_SC_IBF to clear */
	if((ret = wait_for_ec(EC_CTRL_PORT, EC_SC_IBF, 0)) < 0)
		return ret;

	/* 3. Write offset address to data port */
	outb((unsigned char)offset, EC_DATA_PORT);

	/* 4. Wait for EC_SC_IBF to clear */
	if((ret = wait_for_ec(EC_CTRL_PORT, EC_SC_IBF, 0)) < 0)
		return ret;

	/* 5. Write data to data port */
	outb((unsigned char)data, EC_DATA_PORT);

	/* 6. Wait for EC_SC_IBF to clear */
	if((ret = wait_for_ec(EC_CTRL_PORT, EC_SC_IBF, 0)) < 0)
		return ret;

	return 0;
}

int adl_bmc_ec_read_device(u8 addr, u8 *dest, int len, unsigned int RegionIndex)
{	
	volatile unsigned short i;

	mutex_lock(&adl_bmc_dev->adl_mutex);

	for (i = 0; i < len; i++)
	{   
		if (ReadEc(RegionIndex, addr + i, &(dest[i])) != 0)
		{
			mutex_unlock(&adl_bmc_dev->adl_mutex);
			return -1;
		}
	}
	mutex_unlock(&adl_bmc_dev->adl_mutex);
#if 0
	printk("%s-- addr %x\n", __func__, addr); 
	for (i=0;i<len;i++)
		printk("%x ", dest[i]); 
	printk("\n"); 
#endif
	return 0;
}
EXPORT_SYMBOL(adl_bmc_ec_read_device);

int adl_bmc_ec_write_device(u8 addr, u8 *src, int len, unsigned int RegionIndex)
{
	volatile unsigned short i;
	
	mutex_lock(&adl_bmc_dev->adl_mutex);
	for (i = 0; i < len; i++)
	{   
		if (WriteEc(RegionIndex, addr + i,src[i]) != 0)
		{
			mutex_unlock(&adl_bmc_dev->adl_mutex);
			return -1;
		}
	}
	mutex_unlock(&adl_bmc_dev->adl_mutex);
#if  0   
	printk("%s addr %x\n", __func__, addr); 
	for (i=0;i<len;i++)
		printk("%x ", src[i]); 
	printk("\n"); 
#endif
	return 0;
}
EXPORT_SYMBOL(adl_bmc_ec_write_device);

static int SMBus_Block_Write_Read_Bmc(unsigned char Address, unsigned char Command,unsigned char Length, unsigned char *Buffer)
{
	unsigned char temp, i;
	unsigned char timeout = 10, retries = 100, SMBusErrorRetry = ERRORRETRY_TIMES;

	temp = inb(SMBHSTS);

	while((temp & SMBHSTS_INUSE_STS) && retries != 0)
	{
		debug_printk("\nIn_USE_Flag SET! Retry!");
		mdelay(timeout);
		retries--;
		temp = inb(SMBHSTS);
	}
	if(retries == 0 && (temp & SMBHSTS_INUSE_STS)){
		debug_printk("\nForce Control of SMB Controller");
	}

	do
	{
		char retry = 100;

		outb(0x02,SMBAUXCTL);	// Enable 32-byte-Buffer

		// Clear Receive Buffer
		inb(SMBHCTL);			// Reset Index counter
		for(i=0; i<32; i++)
			outb(0x00,SMBHBD);

		inb(SMBHCTL);				// Reset Index counter
		outb(Address, SMBTSA);		// Set Transmit Slave Address (0=Write)
		outb(Command, SMBHCMD);		// Set Host Command
		outb(Length, SMBHD0);		// Clear Number of received bytes or set Bytes to send

		if (BMC_IsWriteAdr(Address))			// If write access: fill buffer
		{
			debug_printk("\nS 0x%02x:+", Command);

			for(i=0; i<Length; i++)
			{
				debug_printk("%02x ", Buffer[i]);
				outb(Buffer[i], SMBHBD);
			}
			debug_printk("+");
			for(i=Length; i<32; i++)
			{
				debug_printk("%02x ", 0x00);
				outb(0x00, SMBHBD);
			}
		}

		temp=inb(SMBHSTS);			// Get current Status
		outb((temp & ~SMBHSTS_INUSE_STS), SMBHSTS);		// Clear all Flags (prepare for send)
		outb(0x14, SMBHCTL);		// Clear block transfer
		outb(0x54, SMBHCTL);		// Start "Block read/write" Command

		do
		{
			mdelay(1);							// short delay while polling busy flag
			temp = inb(SMBHSTS);

			if( !( retry-- ) )
				break;
		}
		while(temp&SMBHSTS_HBSY);				// Wait while host busy (transfer in progress)

		if( INT_SMI_FAILED( temp ) )
			outb(( SET_INT_SMI_FAILED_BIT( temp ) & ~SMBHSTS_INUSE_STS ), SMBHSTS);

		if( BUS_ERROR( temp ) )
			outb(( SET_BUS_ERROR_BIT( temp ) & ~SMBHSTS_INUSE_STS ), SMBHSTS);

		if( TIMEOUT_ERROR( temp ) )
			outb(( SET_TIMEOUT_ERROR_BIT( temp ) & ~SMBHSTS_INUSE_STS ), SMBHSTS);

		outb((temp & 0xff ), SMBHSTS);

	}
	while( SMBUS_ERROR( temp ) && SMBusErrorRetry-- );

	if( SMBUS_ERROR( temp ) )
		return -1;

	temp=inb(SMBHD0);			// Get number of received Bytes

	if(BMC_IsReadAdr(Address))				// If read access: get buffer
	{

		debug_printk("\nR 0x%02x:#", Command);

		if (temp>32)						// read at most 32 bytes
			temp=32;

		for(i=0; i<temp; i++)
		{
			Buffer[i] = inb(SMBHBD);
			debug_printk("%02x ", Buffer[i]);
		}

		for(i=0; i<temp; i++)
		{
			debug_printk("%c", (Buffer[i]>=32&&Buffer[i]<127)?Buffer[i]:'.');
		}
		for( ;i<32; i++)
		{
			debug_printk("-- ");
			inb(SMBHBD);
		}

		debug_printk("#\n");
	}

	outb(0xFF, SMBHSTS);		// Clear all Flags

	return temp;
}

static int SMBus_Trans(unsigned char Address, tTransType Type, unsigned char *Buffer)
{
	unsigned char temp,ctl = 0,Count = 0;
	unsigned char timeout = 10, i,SMBusErrorRetry = ERRORRETRY_TIMES;

	do
	{
		char retry = 100;
		unsigned char retries = 100;

		temp = inb(SMBHSTS);

		while((temp & SMBHSTS_INUSE_STS) && retries != 0)
		{
			debug_printk("\nByte In_USE_Flag SET! Retry!");
			mdelay(timeout);
			retries--;
			temp = inb(SMBHSTS);
		}
		if(retries == 0 && (temp & SMBHSTS_INUSE_STS)){
			debug_printk("\nForce Control of SMB Controller");
		}

		outb(0x02,SMBAUXCTL);	// Enable 32-byte-Buffer

		// Clear Receive Buffer
		inb(SMBHCTL);			// Reset Index counter
		for(i=0; i<32; i++)
			outb(0x00,SMBHBD);

		inb(SMBHCTL);			// Reset Index counter
		outb((TT_IsReadAccess(Type))?Address|1:Address&0xfe,SMBTSA);	// Set Transmit Slave Address (0=Write)

		switch (Type)
		{
			case TT_WB:	outb(Buffer[0],SMBHCMD);
					ctl=0x04;
					Count = 1;
					break;
			case TT_WBB:	outb(Buffer[0],SMBHCMD);
					outb(Buffer[1],SMBHD0);
					ctl=0x08;
					Count = 1;
					break;
			case TT_WBW:	outb(Buffer[0],SMBHCMD);
					outb(Buffer[1],SMBHD0);
					outb(Buffer[2],SMBHD1);
					ctl=0x0C;
					Count = 2;
					break;
			case TT_WWB:	return 0;								// *unsupported transfer mode*

			case TT_RB:	ctl=0x04;
					break;
			case TT_RBB:	outb(Buffer[0],SMBHCMD);
					ctl=0x08;
					Count = 1;
					break;
			case TT_RBW:	outb(Buffer[0],SMBHCMD);
					ctl=0x0C;
					Count = 1;
					break;
			case TT_RWB:	return 0;								// *unsupported transfer mode*
		}

		temp=inb(SMBHSTS);			// Get current Status
		outb((temp & ~SMBHSTS_INUSE_STS), SMBHSTS);		// Clear all Flags (prepare for transfer)
		outb(ctl,SMBHCTL);			// Prepare transfer
		outb(ctl | 0x40,SMBHCTL);	// Start transfer
		do
		{
			mdelay(1);							// short delay while polling busy flag
			temp = inb(SMBHSTS);

			if( !( retry-- ) )
				break;
		}
		while (temp&SMBHSTS_HBSY);				// Wait while host busy (transfer in progress)

		if( INT_SMI_FAILED( temp ) )
			outb(( SET_INT_SMI_FAILED_BIT( temp ) & ~SMBHSTS_INUSE_STS ), SMBHSTS);

		if( BUS_ERROR( temp ) )
			outb(( SET_BUS_ERROR_BIT( temp ) & ~SMBHSTS_INUSE_STS ), SMBHSTS);

		if( TIMEOUT_ERROR( temp ) )
			outb(( SET_TIMEOUT_ERROR_BIT( temp ) & ~SMBHSTS_INUSE_STS ), SMBHSTS);

		outb((temp & 0xff ), SMBHSTS);
	}
	while( SMBUS_ERROR( temp ) && SMBusErrorRetry-- );

	if( SMBUS_ERROR( temp ) )
		return -1;

	outb((temp & ~SMBHSTS_INUSE_STS), SMBHSTS);

	switch (Type)
	{
		case TT_WB:	break;									// nothing to do
		case TT_WBB:	break;									// nothing to do
		case TT_WBW:	break;									// nothing to do
		case TT_WWB:	break;									// *unsupported transfer mode*

		case TT_RB:
		case TT_RBB:	Buffer[0] = inb(SMBHD0);
				Count = 1;
				break;

		case TT_RBW:	Buffer[0] = inb(SMBHD0);
				Buffer[1] = inb(SMBHD1);
				Count = 2;
				break;

		case TT_RWB:	break;									// *unsupported transfer mode*
	}

	outb(0xFF,SMBHSTS);		// Clear all Flags
	return Count;
}

int adl_bmc_i2c_read_device(struct adl_bmc_dev *adl_bmc, char reg,
		int bytes, void *dest)
{
	int ret;

	mutex_lock(&adl_bmc_dev->adl_mutex);

	ret = SMBus_Block_Write_Read_Bmc(BMC_ADDRESS | 0x01, reg, bytes,dest);

	if (ret < 0){
		mutex_unlock(&adl_bmc_dev->adl_mutex);
		debug_printk("return value is  %d\n", ret);
	}

	mutex_unlock(&adl_bmc_dev->adl_mutex);
	return ret;
}

EXPORT_SYMBOL_GPL (adl_bmc_i2c_read_device);

int adl_bmc_i2c_write_device(struct adl_bmc_dev *adl_bmc, int reg,
		int bytes, void *src)
{
	int ret;

	mutex_lock(&adl_bmc_dev->adl_mutex);

	ret = SMBus_Block_Write_Read_Bmc(BMC_ADDRESS, reg, bytes,src);

	if (ret < 0){
		debug_printk("return value is  %d\n", ret);
	}

	mutex_unlock(&adl_bmc_dev->adl_mutex);
	return ret;
}

EXPORT_SYMBOL_GPL (adl_bmc_i2c_write_device);

int adl_bmc_smbus_write_read_trans(int Address, int type, void *src)
{
	int ret;

	mutex_lock(&adl_bmc_dev->adl_mutex);

	if(BMC_IsWriteAdr(Address))
	{
		ret = SMBus_Trans(Address & 0xfe, type,src);
	}
	else
	{
		ret = SMBus_Trans(Address | 0x01, type,src);
	}

	if (ret < 0){
		debug_printk("return value is  %d\n", ret);
		mutex_unlock(&adl_bmc_dev->adl_mutex);
		return ret;
	}

	mutex_unlock(&adl_bmc_dev->adl_mutex);
	return 0;
}

EXPORT_SYMBOL_GPL (adl_bmc_smbus_write_read_trans);

void CollectCapabilities(unsigned int *Capabilities, unsigned DataCount, unsigned char *SMBusDatas)
{
	if(adl_bmc_dev->con_type == EC)
	{
		unsigned char buff[8] = {0};
		adl_bmc_ec_read_device(ADL_BMC_OFS_CAPABILITIES, buff, 8, EC_REGION_1);

		if(Capabilities != NULL)
		{
			unsigned char i;
			for(i = 0; i < 8; i++)
			{
				Capabilities[i/4] |= buff[i] << ((i%4) * 8);
				//printk("capability%d=%x\t", i, buff[i] );
			}
		}
	}
	else
	{
		unsigned char   ShiftNumbers[] = { SHIFT( 24 ), SHIFT( 16 ), SHIFT( 8 ), SHIFT( 0 ) };
		unsigned        CapabilityCount = DataCount / CAPABILITY_BYTE_UNIT + ( DataCount % CAPABILITY_BYTE_UNIT ? ONE_BYTE : ZERO_BYTE );
		unsigned        RemainderData = DataCount;
		unsigned        Loop, Loop1;
		unsigned        CapabilityIndex = CAPABILITY_INDEX( CapabilityCount ), ShiftIndex = SHIFT_INDEX( DataCount ), DataIndex = 0;
		if( !CapabilityCount || DataCount > MAX_BUFFER_SIZE )
			return;

		for( Loop = 0 ; Loop < CapabilityCount ; ++Loop, --CapabilityIndex )
		{ // Collect all
			unsigned        CheckDataCount = RemainderData % CAPABILITY_BYTE_UNIT;
			unsigned        CombineData = CheckDataCount ? CheckDataCount : CAPABILITY_BYTE_UNIT;
			RemainderData -= CombineData;
			for( Loop1 = 0 ; Loop1 < CombineData ; ++Loop1, ++DataIndex )
			{ // Combine all
				*( Capabilities + CapabilityIndex ) |= ((unsigned int)(*( SMBusDatas + DataIndex )) << (*( ShiftNumbers + ShiftIndex )));
				ShiftIndex = (unsigned char)( (ShiftIndex + 1) % 4);
			} // Combine all
		} // Collect all
	}
}

EXPORT_SYMBOL_GPL (CollectCapabilities);

static int hello_proc_show(struct seq_file *m, void *v) {
	seq_printf(m, "EC\n");
	return 0;
}

static int hello_proc_open(struct inode *inode, struct  file *file) {
	return single_open(file, hello_proc_show, NULL);
}

static const struct file_operations ops = {
	.owner = THIS_MODULE,
	.open = hello_proc_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};

static dev_t sema;
static struct cdev cdev;
static struct class *cl;

int StatusCheck(void)
{
	unsigned char status, i;

	for(i=0; i<100; i++)
	{
		if(adl_bmc_ec_read_device(EC_RW_ADDR_IIC_BMC_STATUS, &status, 1, EC_REGION_2) == 0)
		{
			if((status & 0x1)==0)
				return 0;
		}
	}
	return -1;
}

static int ReadMem_bmc(unsigned char Region, unsigned int nAdr, u8* pData, unsigned int nSize)
{
	
	unsigned char pDataIn[] = { 0x2, 0x1, (unsigned char)nSize, (unsigned char)(Region + 1), (unsigned char)(nAdr >> 8), (unsigned char)(nAdr & 0xFF) };

	if(StatusCheck()!=0)
	{
		return -1;	    
	}

	if (adl_bmc_ec_write_device(EC_WO_ADDR_IIC_CMD_START,pDataIn, 6, EC_REGION_2) == 0)
	{
		pDataIn[0] = 4;

		if (adl_bmc_ec_write_device(EC_RW_ADDR_IIC_BMC_STATUS, pDataIn, 1,EC_REGION_2) == 0)
		{
			unsigned char i;
			for (i = 0; i < 100; i++)
			{
				if (adl_bmc_ec_read_device(EC_RW_ADDR_IIC_BMC_STATUS, pDataIn, 1,EC_REGION_2) == 0)
				{
					if (!!(pDataIn[0] & 0x04) == 0 && ((pDataIn[0] & 0x1) != 1) && (!!(pDataIn[0] & 0x8) != 1))
					{
						delay(40);
						adl_bmc_ec_read_device(EC_RW_ADDR_IIC_BUFFER, pData, nSize,EC_REGION_2);
						return 0;
					}
				}
				delay(100);
			}
		}
	}

	return -1;
}

static int bmc_nvmem_read(unsigned int offset,void *val, size_t bytes)
{
	size_t size;
	int ret=0, i;
	size = bytes;

	if(val == NULL)
	{
		return -1;
	}

	offset = offset + 0x1800;

	for(i = 0; size > 0; i += 32)
	{
		if(size > 32)
		{
			delay(200);
			size -= 32;
		}
		else
		{
			delay(200);
			ret = ReadMem_bmc(5, offset + i, (char*)val + i, size);
			size =0;		
		}

		if (ret < 0)
		{
			printk("Error in ReadMem function\n");
			return ret;
		}
	}

	return ret;
}

static int IsECDeviceExist(void)
{
	char buffer[50] = {0};
	unsigned char status;
	
	if (adl_bmc_ec_read_device(0xF0, buffer, 16, EC_REGION_2) >= 0)
    	{
    		debug_printk(KERN_INFO "ADLINK Info: EC Firmware Supports Dual Region Access\n");
    		ec_region = EC_REGION_2;
    	}
    	else
    	{
    		ec_region = EC_REGION_1;
    		if (adl_bmc_ec_read_device(0xF0, buffer, 16, EC_REGION_1) < 0)
    		{
        		return -1;
    		}
    		debug_printk(KERN_INFO "ADLINK Info: EC Firmware Supports Single Region Access\n");
    	}

	if(strstr(buffer, "ADLINK") == NULL)
	{
		/*Reading the address 0xEA*/ 
		if (adl_bmc_ec_read_device(0xEA, &status, 1, EC_REGION_1) < 0)
		{		
			return -1;
		}
		/*Checking the address 0xEA for value 0x34*/ 
		if (status != 0x34)
		{
			return -1;
		}
	}
	return 0;
}

static int IsSMBDeviceExist(void)
{
	struct pci_dev *pdevv = NULL;
	int node;

	for(node = 0; node < NUM_SMBUS_CHIPSETS; node++)
	{
		struct SMBusInfo boardinfo = SMBUS_ChipsetInfo[node];  
		pdevv = pci_get_device(boardinfo.Vendor_id, boardinfo.Device_id, NULL);

		if(pdevv)
		{
			if(!pci_enable_device(pdevv))
			{
				SMBBase = pci_resource_start(pdevv,4);
				pci_disable_device(pdevv);
				return 0;
			}
		}
	}
	return -1;
}

static int adl_bmc_acpi_probe(struct platform_device *pdev)
{
	// check EC interface is valid
	struct path path;
	int i, found = 0, ret;
	char buffer[50] = {0};
	unsigned char *pData;
	uint8_t hardware_monitor;
	unsigned char buf[32];
	adl_bmc_dev = devm_kzalloc(&(pdev->dev), sizeof(struct adl_bmc_dev), GFP_KERNEL);

	/**Mutex Init**/
	mutex_init(&adl_bmc_dev->adl_mutex);

	if (adl_bmc_dev == NULL)
		return -ENOMEM;
	
	if(IsECDeviceExist() == 0)
	{
		adl_bmc_dev->con_type = EC;
		adl_bmc_dev->region = ec_region;
	}
	else if(IsSMBDeviceExist() == 0)
	{
		adl_bmc_dev->con_type = BMC;
	}
	else
	{
		return -ENODEV;
	}

	adl_bmc_dev->dev = &pdev->dev;
	debug_printk("==> %s\n",__func__);

	if(adl_bmc_dev->con_type == EC)
	{
		struct pci_dev *base = NULL;
		printk("[ADLINK] SEMA FOR EC\n");
		base = pci_get_class(PCI_CLASS_SERIAL_SMBUS << 8, NULL);

		if(base)
		{
			if(!pci_enable_device(base))
			{
				SMBBase = pci_resource_start(base,4);
				pci_disable_device(base);
			}
		}
		if(kern_path ("/dev/sema", LOOKUP_FOLLOW, &path) == 0)
		{
			return -ENODEV;
		}

		if(alloc_chrdev_region(&sema, 0, 1, "sema") < 0)
		{
			return -ENODEV;
		}

#if LINUX_VERSION_CODE >= KERNEL_VERSION(5,14,0) && defined(CONFIG_REDHAT)
		if((cl = class_create("sema")) == NULL)
#elif LINUX_VERSION_CODE >= KERNEL_VERSION(6,4,0)
		if((cl = class_create("sema")) == NULL)
#else
		if((cl = class_create(THIS_MODULE, "sema")) == NULL)
#endif
		{
			unregister_chrdev_region(sema, 1);
			return -ENODEV;
		}

		if(device_create(cl, NULL, sema, NULL, "sema") == NULL)
		{
			class_destroy(cl);
			unregister_chrdev_region(sema, 1);
			return -ENODEV;
		}

		cdev_init(&cdev, &ops);

		if(cdev_add(&cdev, sema, 1) < 0)
		{
			device_destroy(cl, sema);
			class_destroy(cl);
			unregister_chrdev_region(sema, 1);
			return -ENODEV;
		}

		if(adl_bmc_ec_read_device(ADL_BMC_OFS_BRD_NAME, buffer, 16, EC_REGION_1) < 0)
		{
			device_destroy(cl, sema);
			class_destroy(cl);
			unregister_chrdev_region(sema, 1);
			return -EIO;
		}

		for(i = 0; i < sizeof(supported_device)/sizeof(supported_device[0]); i++)
		{
			if(strstr(buffer,supported_device[i].device_name) != NULL)
			{
				adl_bmc_dev->current_board = supported_device[i];
				found = 1;
				break;
			}
		}

		if(found == 0)
		{
			adl_bmc_dev->current_board = default_device;
		}

		adl_bmc_dev->CollectCapabilities = CollectCapabilities;

		/* mutex init */
		mutex_init(&adl_bmc_dev->mx_nvmem);

		CollectCapabilities(adl_bmc_dev->Bmc_Capabilities, 0,  NULL);

		debug_printk("%x %x\n", adl_bmc_dev->Bmc_Capabilities[0], adl_bmc_dev->Bmc_Capabilities[1]);


		debug_printk("<== %s\n",__func__);

		hardware_monitor = (adl_bmc_dev->Bmc_Capabilities[1]& 0x0800) ? true:false; //To get 43rd bit
											    //printk("Bmc_Capabilities(0x15) %x, hardware_monitor %x\n", adl_bmc_dev->Bmc_Capabilities[1], hardware_monitor);

		if(hardware_monitor==1){
			
			for (i=0; i<8;i++)
			{

				pData = devm_kzalloc(&(pdev->dev), (sizeof(unsigned char) *16), GFP_KERNEL);

				if((bmc_nvmem_read( (i*0x10), pData, 16) )<= 0){
					adl_bmc_dev->current_board.voltage_description[i]=pData;
				}
				else{
					printk("Error reading Hardware Monitor input string\n");
					break;
				}
			}
		}	
	}
	else
	{
		memset(buf, 0, sizeof(buf));

		ret = adl_bmc_i2c_read_device(NULL, ADL_BMC_CMD_CAPABILITIES, 0x00, buf);

		if (ret < 0)
			return ret;

		for(i = 0;i < 32;i++)
		{
			debug_printk("%d-> %x: \n",i,buf[i]);
		}

		printk("[ADLINK] SEMA FOR BMC\n");
		//buf[0] contains size
		//MSB is first
		CollectCapabilities(adl_bmc_dev->Bmc_Capabilities, ret, buf);
	}
	platform_set_drvdata(pdev, adl_bmc_dev);

	return mfd_add_devices(adl_bmc_dev->dev, -1, adl_bmc_devs, ARRAY_SIZE(adl_bmc_devs), NULL, 0, NULL);
}

#if LINUX_VERSION_CODE >= KERNEL_VERSION(6,11,0)
static void adl_bmc_acpi_remove(struct platform_device *pdev)
#else
static int adl_bmc_acpi_remove(struct platform_device *pdev)
#endif
{
	debug_printk("==> %s\n",__func__);
	if(adl_bmc_dev->con_type == EC)
	{
		cdev_del(&cdev);
		device_destroy(cl, sema);
		class_destroy(cl);
		unregister_chrdev_region(sema, 1);

		mfd_remove_devices (adl_bmc_dev->dev);
	}
	else
	{
		mfd_remove_devices(adl_bmc_dev->dev);
		SMBBase = 0x00;
		debug_printk("Remove............\n");

	}
#if LINUX_VERSION_CODE < KERNEL_VERSION(6,11,0)
	return 0;
#endif
}


static struct platform_driver adl_bmc_acpi_driver = {
	.driver = {
		.name	= "adl-bmc-acpi",
	},

	.probe		= adl_bmc_acpi_probe,
	.remove		= adl_bmc_acpi_remove,
};

static void adl_bmc_acpi_release (struct device *dev)
{
	return;
}

static struct platform_device adl_bmc_acpi_device = {
	.name = "adl-bmc-acpi",
	.id = -1,
	.dev = {
		.release = adl_bmc_acpi_release,
		.platform_data = NULL,
	}
};

static int __init adl_bmc_acpi_init (void)
{
	if(platform_device_register(&adl_bmc_acpi_device) == 0)
		return platform_driver_register(&adl_bmc_acpi_driver);
	else
		platform_device_unregister(&adl_bmc_acpi_device);

	return -1;
}

static void __exit adl_bmc_acpi_exit (void)
{
	platform_device_unregister(&adl_bmc_acpi_device);
	platform_driver_unregister(&adl_bmc_acpi_driver);
}

module_init(adl_bmc_acpi_init);
module_exit(adl_bmc_acpi_exit);

MODULE_LICENSE("Dual BSD/GPL");
MODULE_AUTHOR("Adlink ");
MODULE_DESCRIPTION("ACPI EC Driver");
